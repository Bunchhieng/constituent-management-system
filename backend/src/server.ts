...
// Middleware
const basicAuth = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
...
const getConstituentsHandler = async (req: Request, res: Response) => {
  ...
  if (typeof search === 'string') {
    query = query.where(db.raw(`first_name ILIKE '%${search}%' OR last_name ILIKE '%${search}%'`));
  }
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to retrieve constituents', details: (error as Error).message });
  }
};
...
const addConstituentHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingConstituent.length > 0) {
      ...
      return res.status(200).json({ message: 'Constituent updated successfully', constituent: updatedConstituent });
    } else {
      ...
      return res.status(201).json({ message: 'Constituent added successfully', constituent: newConstituent });
    }
  } catch (error) {
    console.error('Error adding/updating constituent:', error);
    res.status(500).json({ error: 'Failed to add/update constituent', details: (error as Error).message });
  }
};
...
const exportConstituentsCSVHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=constituents_${from}_to_${to}.csv`);
    res.status(200).send(csv);
  } catch (error) {
    console.error('Error exporting constituents:', error);
    res.status(500).json({ error: 'Failed to export constituents', details: (error as Error).message });
  }
};
...
const updateConstituentStatusHandler = async (req: Request, res: Response) => {
  ...
  try {
    const updateResult = await db('constituents').where('id', id).update('status', status);
    if (updateResult === 0) {
      return res.status(404).json({ error: 'Constituent not found' });
    }
    res.status(200).json({ message: 'Constituent status updated successfully' });
  } catch (error) {
    console.error('Error updating constituent status:', error);
    res.status(500).json({ error: 'Failed to update constituent status', details: (error as Error).message });
  }
};
...
const getTopIssuesHandler = async (req: Request, res: Response) => {
  try {
    const topIssues = await db.select({
      id: issues.id,
      name: issues.name,
      count: issues.count
    })
    ...
    res.status(500).json({ error: 'Failed to fetch top issues', details: (error as Error).message });
  }
};
...
app.post('/constituents', [
  check('firstName').not().isEmpty(),
  check('lastName').not().isEmpty(),
  check('email').isEmail().withMessage('Invalid email'),
  check('phone').optional().isMobilePhone('any', { strictMode: false }).withMessage('Invalid phone number'),
